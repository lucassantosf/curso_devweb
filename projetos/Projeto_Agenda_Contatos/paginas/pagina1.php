...
<body>
			...
			<div class="col-md-6">
				<div class="panel panel-info">
					<div class="panel-heading">
						<h4>Contatos Existentes<a href="cadastrarContato.php" style="margin-left: 10px;width: 20px;height: 20px;"><span style="margin-left: -5px;" class="glyphicon glyphicon-plus"></span></a></h4>
					</div>
					<div class="panel-body" id="contatos_existentes">
						<?php
							$consultarContatos = "select * from tb_contatos";
							$resultado_query = mysqli_query($link,$consultarContatos);
							if($resultado_query){
								while ($registro = mysqli_fetch_array($resultado_query, MYSQLI_ASSOC)) {
									echo $registro['descricao'].' - '.$registro['telefone'];	
									echo '<a onclick="deletar('.$registro['id_contato'].')"; style="margin-left:5px;">';
	...
	<script type="text/javascript">
		...
		function deletar(id_contato){
			var request = $.ajax({
		      	url: "http://localhost:8080/php/Login_section/paginas/deletarContato.php",
		      	method: "POST",
		    ...
		    request.done(function( msg ){
		        document.getElementById('contatos_existentes').innerHTML = location.reload();
		    });
